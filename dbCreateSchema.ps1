...
$ORACLE_DBSCRIPTS="$ORACLE_BASE\oraSetupScripts\BullHorn"
$ORACLE_SID="ZDBT01W"
...
{ 
    ...
    #
    &$ORACLE_BIN\sqlplus system/$SYSTEM_PASS @$ORACLE_DBSCRIPTS\ZDBT01W_schema_bh01_db.sql
	
	# 2. Create BH Tables / Load data
    #
    &$ORACLE_BIN\sqlplus bhdba/bhdba @$ORACLE_DBSCRIPTS\ZDBT01W_load_bh01_db.sql
	
    # 3. Grant permissions for our DB
    #
    &$ORACLE_BIN\sqlplus bhdba/bhdba @$ORACLE_DBSCRIPTS\ZDBT01W_grant_bh01_db.sql
}

# Create Directory for our data files if not already there
$appDataDir="${ORACLE_BASE}\oradata\$ORACLE_SID\bh"
